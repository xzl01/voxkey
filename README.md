...
![Qwen Voice Input architecture](docs/assets/qwen-voice-input-architecture.svg)

Qwen Voice Input 是一个面向 Linux / Wayland 的本地语音输入工具。用户显式绑定触发键后，按住按键开始录音，松开后调用本地 Qwen3-ASR-GGUF 转写，并把文本提交到当前聚焦输入框。
...
│   ├── install-user.sh
│   └── README.md
├── systemd/user/qwen-voice-input.service
├── tests/test_voice_input_daemon.py
├── docs/
│   ├── assets/qwen-voice-input-architecture.svg
...
- [Arch Linux + niri 语音输入部署记录](docs/qwen-voice-input-arch-niri.md)
