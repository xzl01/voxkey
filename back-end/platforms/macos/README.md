...
`setup.sh` 会自动编译并检查这两个程序。仅在修改 Swift 源码后需要手动重编：
...
> **桌面 App（DMG）**：权重不随 DMG 打包。App 首次启动后从 GitHub Release
> `voxkey-models-v1` 按需下载（FunASR `funasr_coreml.tar.gz` + Qwen3 GGUF），
> 并校验 `manifests/` 中的 SHA-256；下载由 `ServicePage` 的「下载模型」按钮触发，
> 也可在 App 内手动重试。权重由 `.github/workflows/publish-models.yml` 转换并发布。
